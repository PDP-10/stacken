...
;Running BLD:
;
;Submit BLD with the following command:
;	.SUBMIT BLD[,]/TAG:TOOL,TOOL
...
.ASSIGN DEC REL
;
.COMPILE/COMP F.MAC,S.MAC,NETPRM.MAC,D36PAR.MAC
.IF (ERROR) .GOTO COMMON
...
.IF (ERROR) .GOTO COMMON
;
.COMPILE/COMP DEC:S.MAC
.IF (ERROR) .GOTO COMMON
;
.COMPILE/COMP DEC:NETPRM.MAC
.IF (ERROR) .GOTO COMMON
;
.COMPILE DLSNUP= DNSNUP.MAC
.IF (ERROR) .GOTO COMMON
;
.R LINK
*DLSNUP/SSAVE=DLSNUP/GO
.IF(ERROR) .GOTO DIR
;
.R DEC:MONGEN
*PROMPT	;HELP
...
.IF (ERROR) .GOTO COMMON
;
.COMPILE DSSNUP= DNSNUP.MAC
.IF (ERROR) .GOTO COMMON
;
.R LINK
*DSSNUP/SSAVE=DSSNUP/GO
.IF(ERROR) .GOTO DIR
;
.GOTO COMMON
DNTATL::
...
.IF (ERROR) .GOTO COMMON
;
.COMPILE DLTATL= DNTATL.MAC
.IF (ERROR) .GOTO COMMON
;
.R LINK
*DLTATL/SSAVE=DLTATL/GO
.IF(ERROR) .GOTO DIR
;
.R DEC:MONGEN
*PROMPT	;HELP
...
.IF (ERROR) .GOTO COMMON
;
.COMPILE DSTATL= DNTATL.MAC
.IF (ERROR) .GOTO COMMON
;
.R LINK
*DSTATL/SSAVE=DSTATL/GO
.IF(ERROR) .GOTO DIR
;
.GOTO COMMON
DTS::
...
.ASSIGN DEC REL
;
.COMPILE/COMP DTSPRM.MAC,DTSCOM.MAC,DTS.MAC
.IF (ERROR) .GOTO COMMON
;
R LINK
...
.ASSIGN DEC REL
;
.COMPILE/COMP DTSPRM.MAC,DTSCOM.MAC,DTR.MAC
.IF (ERROR) .GOTO COMMON
;
R LINK
...
.ASSIGN DEC REL
;
.COMPILE/COMP DPYDEF.MAC,DPY.MAC,KDPDPY.MAC
.IF (ERROR) .GOTO DIR
;
.R LINK
...
.IF (ERROR) .GOTO DIR
.R LINK
*TATTLE/SSAVE=TATSUB,TATTLE/OTS:NONSHARE/SEG:LOW/GO
.IF (ERROR) .GOTO DIR
;
.GOTO COMMON

