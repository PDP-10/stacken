;SOSFOR.CTL  %007   MAKE AND RUN SOSFOR.SCP WHICH SIMULATES A LOAD OF
;USERS DOING SOS, FORTRAN AND DDT.
;THE .CTL FILE RUNS THE SCRIPT TWICE AS FAST AS POSSIBLE.  10
;COPIES ARE RUN, STAGGERED. TO RUN SOSFOR TYPE:
;	SUBMIT SOSFOR/TIME:700,/DIS:DEL
;SOSFOR IS USUALLY RUN BY MONTST.CTL IN WEEKLY MONITOR TEST
;SM LIBMAN	 4 AUG 77  
;
.GOTO SKIP
SCP::
!;;STANDARD BENCHMARK SCRIPT FOR SOS/FORTRAN (WITH SLOW USER) - %007
!;;SM LIBMAN /SML  4 AUG 77
!;;FOR 603 MONITOR
...
GET SYS:LOADER
VERSION
!;; WE WANT TO RUN SOSX IF IT IS THERE, ELSE NORMAL (?) SOS
COPY = SYS:SOS.EXE
!Q
COPY SOS.EXE = SYS:SOSX.EXE
!Q
DELETE ROOTS.*,SOURCE.*
RUN SOS - SOURCE.F4
!2000R 25000F
...
=.,ZKPWEF
;
;END OF FILE SOSFOR.SCP.  RUN 10 JOBS.
;
...
*Y
.IF (ERROR) .GOTO B
.PLEASE SOSFOR SUCCESSFUL
.DELETE SOSFOR.WCH
.DELETE SOSFOR.SCP
.GOTO A
B::
.PLEASE ERROR RUNNING SOSFOR.SCP
.PRINT SOSFOR.WCH/DISPOSE:DELETE
A::
%FIN:
.NOERROR
.DELETE SOSFOR.SCP
